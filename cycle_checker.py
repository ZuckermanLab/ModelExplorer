...
def import_and_format_data(ADJACENCY_FILE,ENERGIES_FILE, TRANSITION_ADJACENCY_FILE, TRANSITION_ENERGY_FILE):
    ...
    raw_adjacency_data = np.genfromtxt(ADJACENCY_FILE, delimiter=',', dtype=None, encoding=None) #guesses data type (is slower)
    raw_energies_data = np.genfromtxt(ENERGIES_FILE, delimiter=',', dtype=None, encoding=None)
...
#initializes variables/flags and controls program flow
...
def main():
    ...
    for k in range(n):
        ...
        master_dictionary1 = copy.deepcopy(master_dictionary)
        C = {}
        i = 0

        # make dictionary with latin square energies for each state/transition
        for item in master_subgraph_list1:
            for sub_item in item:
                C[sub_item]= B[k][i]  # use first latin square sample point (for now)
            i = i + 1

        # go through each key in tied dictionary and update with energies from latin square
        for key in master_dictionary1.keys():
            #print(key)
            for sub_key in master_dictionary1[key]:
                #print(sub_key)
                #print(master_dictionary[key][sub_key])
                temp = master_dictionary1[key][sub_key]
                master_dictionary1[key][sub_key] = temp + C[sub_key]
            #print("\n")
    ...
    for k in range(n):
        C = {}
        i = 0
        master_transition_dictionary1 = copy.deepcopy(master_transition_dictionary)
        master_transition_subgraph_list1 = copy.deepcopy(master_transition_subgraph_list)
        # make dictionary with latin square energies for each state/transition
        for item in master_transition_subgraph_list1:
            for sub_item in item:
                C[sub_item]= B[k][i]  # use first latin square sample point (for now)
            i = i + 1

        # go through each key in tied dictionary and update with energies from latin square
        for key in master_transition_dictionary1.keys():
            #print(key)
            for sub_key in master_transition_dictionary1[key]:
                #print(sub_key)
                #print(master_dictionary[key][sub_key])
                temp = master_transition_dictionary1[key][sub_key]
                master_transition_dictionary1[key][sub_key] = temp + C[sub_key]
            #print("\n")
        # match with key in latin square dictionary
        # add latin square value to each value in dictionary
